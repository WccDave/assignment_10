...
$elementsArr = [
  ...
  "age"=>[
    "errorMessage"=>"<span style='color: red; margin-left: 15px;'>You must select one age group</span>",
    "errorOutput"=>"",
    "action"=>"required",
    "type"=>"radio",
...
function addData($post){
  ...
  /* IF EVERYTHING WORKS ADD THE DATA HERE TO THE DATABASE HERE USING THE $_POST SUPER GLOBAL ARRAY */
      ...
      $sql = "INSERT INTO contactMod (name, address, city, state, phone, email, updates, age) VALUES (:name, :address, :city, :state, :phone, :email, :updates, :age)";
...
<h1>Add Contact</h1>
    ...
    <div class="form-check form-check-inline">
      ...
      <label class="form-check-label" for="updates1">News Letter</label>
    </div>
    <div class="form-check form-check-inline">
      <input class="form-check-input" type="checkbox" name="updates[]" id="updates2" value="emailUpdates" {$elementsArr['updates']['status']['emailUpdates']}>
      <label class="form-check-label" for="updates2">Email</label>
    </div>
    <div class="form-check form-check-inline">
      <input class="form-check-input" type="checkbox" name="updates[]" id="updates3" value="textUpdates" {$elementsArr['updates']['status']['textUpdates']}>
